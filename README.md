...
Agents help a lot when the project’s truth lives in the repo—not in whatever someone said in chat last Tuesday. A useful session should leave you (or the next agent) with a clearer project than it started with. Confident fiction and half-remembered decisions do the opposite.

In practice that means:

- **Get the process straight before faking a product.** The early win is knowing what’s known, what’s guessed, what to read next, and what “done” is allowed to mean—not a shiny mock or invented API that looks finished.
- **Look around before making things up.** Early work is for gathering facts and open questions. Named endpoints, version pins, and compliance claims wait for evidence or a real decision. If you have to assume, label it as an assumption.
- **Different docs do different jobs.** The constitution and `AGENTS.md` hold the hard rules. Specs hold the living truth for a topic (with claim tables). ADRs record decisions you can’t quietly undo, including opening a stage. The tasks file tracks status and where evidence lives—not long essays. The registry says which docs matter for the task you’re on.
- **Don’t load the whole docs tree every time.** Stuffing every file into context makes the agent blur them together. Read the short list for the current task and stay there.
- **“Done” needs proof.** Outside a few explore/process exceptions, that means docs match reality, tests pass, and something actually ran in this session. Sounding finished doesn’t count.
- **When you guess wrong, leave a trail.** Fix the spec. Write an ADR or a registry note if the course changed. If the same mistake can happen again, add a test or a structural check. Don’t silently rewrite history and hope the next session notices.

Ideas can be messy and energetic. What’s in the repo should still be checkable: citations, pins, stage authorizations, and commands that ran. This kit won’t make an agent follow the docs—but it makes the honest path the obvious one.

More detail: [references/method.md](references/method.md). Defaults for the hard rules: [references/constitution.md](references/constitution.md).

---

## Rules (after bootstrap)

These are the same ideas as everyday working rules. After bootstrap, treat them as the project contract—not optional style tips.
